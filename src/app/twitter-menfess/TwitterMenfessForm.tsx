...
export default function TwitterMenfessForm({}) {
  ...
  const userId = getCookie("twitterId");
  ...
  const handlePostTweet = async (e: React.MouseEvent<HTMLButtonElement>) => {
    ...
    if (tweet.search(/Cjr!/) >= 0) {
      ...
      if (id) {
        ...
        await Airtables("CjrFessTweet").create([
          {
            fields: {
              link: id,
              tweet: tweet,
  ...
  return (
    <>
      <div className="flex flex-col gap-3 lg:justify-start justify-center rounded-lg shadow-md bg-white p-5">
        <Direction />
        <div className="flex">
      ...
      <Response success={isSuccess} refetch={refetch} />
    </>
  );
}
